...
function makeNumberPattern(): Pattern {
  ...
  if (type === 'add') {
    const start = Math.floor(Math.random() * 5) + 1, step = Math.floor(Math.random() * 4) + 1
    seq = Array.from({ length: 5 }, (_, i) => start + i * step); answer = start + 5 * step
  } else if (type === 'mul') {
    const factor = Math.floor(Math.random() * 2) + 2
    seq = Array.from({ length: 5 }, (_, i) => Math.pow(factor, i)); answer = Math.pow(factor, 5)
  } else {
  ...
  const explanation = type === 'add' ? `+${seq[1] - seq[0]} ずつふえる！` : type === 'mul' ? `×${Math.round(seq[1] / seq[0])} ずつ！` : 'まえの2つをたすフィボナッチ数列！'
  return { seq: seq.map(String), answer: ansStr, choices, explanation }
}
...
export function WhatsNext() {
  ...
  function start(d: Difficulty) { setDiff(d); setScore(0); setQNum(1); setPattern(makePattern(d)); setPhase('play') }

  function tap(c: string) {
    if (!pattern) return
    if (c === pattern.answer) { setFlash('ok'); setScore(s => s + 1); setShowExp(false) }
      ...
      <div className="flex flex-col gap-4 pt-6">
        ...
        {(['easy', 'hard'] as Difficulty[]).map(d => (
          <button key={d} onClick={() => start(d)} className="bg-white border border-indigo-100 rounded-2xl p-5 text-left shadow-md active:scale-95">
            ...
            <p className="text-sm text-gray-500 mt-1">{d === 'easy' ? '2つのくりかえし（いろ・かたち・かず）' : '3〜4つのくりかえし ＋ すうれつ'}</p>
          </button>
        ))}
      </div>
    </GameLayout>
      ...
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="grid grid-cols-2 gap-3 w-full">
          {pattern.choices.map((c, i) => (
            <button key={i} onClick={() => tap(c)} className="bg-white rounded-2xl border-2 border-indigo-200 shadow-md active:scale-95 transition-transform font-bold text-gray-800" style={{ height: 80, fontSize: 32 }}>
              {c}
            </button>
          ))}
        </div>
